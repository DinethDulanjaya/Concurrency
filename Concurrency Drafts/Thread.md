...
int main()
{
    ...
    auto f0 = [&id]() { std::cout << "a) ID in Lambda 1= " << id << std::endl; };

    // capture by value (mutable)
    auto f1 = [id]() mutable { std::cout << "b) ID in Lambda 2= " << ++id << std::endl; };
    f1(); // call the closure and execute the code witin the Lambda
    std::cout << "c) ID in Main 1= " << id << std::endl;

    // capture by reference (mutable)
    auto f2 = [&id]() mutable { std::cout << "d) ID in Lambda 3= " << ++id << std::endl; };
    f2(); 
    std::cout << "e) ID in Main 2= " << id << std::endl; 

    // pass parameter 
    auto f3 = [](const int id) { std::cout << "f) ID in Lambda 4= " << id << std::endl; };   
    f3(++id);
...
int main()
{
    ...
    std::shared_ptr<Vehicle> v(new Vehicle);
    std::thread t = std::thread(&Vehicle::addID, v, 1); // call member function on object v
    std::thread t1 = std::thread(&Vehicle::setName, v, "Ford");
